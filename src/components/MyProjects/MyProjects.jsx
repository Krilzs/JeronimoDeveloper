...
const MyProjects = ()=>{
    ...
    return(
        <article className="flex flex-col bg-[#111] p-8 w-full lg:px-25 pt-25" id="mi-projects">
...
const Project = (project)=>{
    ...
    const handleMoreInfo = ()=>{
        buttonText=="Descripcion" ? setButtonText("Ocultar") : setButtonText("Descripcion")
        setMasInfo(!masInfo)
    }
        ...
        <div className="w-full flex flex-col bg-[#222] text-white shadow-2xl/5 shadow-[#00ffaa]  rounded-xl transition-all duration-500 ease-in-out lg:h-fit lg:w-[%33] lg:">
            {
                projectsImages.map((img)=>{
                    if (img.includes(titulo)){          
                        ...
                        return(
                            <img className="rounded-t-xl w-full object-cover h-40 lg:h-55" src={img} alt={titulo} key={titulo}/>
                        )
                    }
                })
            }
            <div className="flex flex-col rounded-b-xl w-full p-2 gap-2 transition-all duration-500 ease-in-out">
                <div className="flex items-center justify-between">
                    <h5 className="flex items-center text-2xl font-[900] w-full break-all lg:min-h-10">{titulo}</h5>
                ...
                <span className="justify-center flex items-center gap-5">
                    {techs.map((tech)=>{
                        return(<img className="flex w-10 h-10 object-contain" src={languages.find((language)=>language.includes(tech))} alt={tech} key={tech}/>)
                    })}
                </span>
                <div className={`${ masInfo ? "h-40 overflow-hidden lg:h-30" : "h-0 overflow-hidden"} flex-col flex items-center justify-center text-justify transition-all duration-500 ease-in-out`}>
                    <p>{descripcion}</p>
                </div>
                <button onClick={handleMoreInfo} className=" cursor-pointer border-t-1 pt-1 self-center w-full text-[#00ffaa] font-[600]"> 
                    {buttonText}
                </button>
            </div>
        </div>
